...
public class Word2VecTest {
  ...
  public void buildGraph() {
    // build graph
    Map<String, Set<WordScanner.VectorDistancePair>> adjacencyList = new HashMap<>();
    Set<String> uniques = WordScanner.valuableTokensOf(earthquake);
    long start = System.currentTimeMillis();
    for (String unique : uniques) {
      adjacencyList.put(unique, WordScanner.mostSimilarToN(unique, uniques, 4));
    }
    ...
    Map<String, Integer> frequencies = WordScanner.wordFrequenciesOf(earthquake);

    adjacencyList.forEach((rootWord, setOfPairs) -> {
      scores.put(rootWord, scores.getOrDefault(rootWord, uniques.size()) - 1);
      setOfPairs.forEach(
          pair -> {
            scores.put(rootWord,
  ...
  private static final String cnn = "Washington (CNN) - President Donald Trump on Sunday again mocked North Korean leader Kim Jong Un and said Secretary of State Rex Tillerson should not bother trying to negotiate with him in an effort to stop the country's development of nuclear weapons.\n"
      + "\n"
  ...
  private static final String interview = "Interviewing as a software engineer is effectively broken. As an interviewer, it’s basically impossible to really determine someone’s skill level, and as an interviewee it’s basically impossible to prove to someone why they should hire you.\n"
      + "\n"
